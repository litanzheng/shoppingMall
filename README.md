...
├── shopping-auth-server -- 认证中心（社交登录、OAuth2.0）
├── shopping-cart -- 购物车服务
├── shopping-coupon -- 优惠卷服务
├── shopping-gateway -- 统一配置网关
├── shopping-order -- 订单服务
├── shopping-product -- 商品服务
├── shopping-search -- 检索服务
├── shopping-seckill -- 秒杀服务
├── shopping-third-party -- 第三方服务（对象存储、短信）
├── shopping-ware -- 仓储服务
└── shopping-member -- 会员服务
```
...
```
192.168.56.102	shopping.com
192.168.56.102	search.shopping.com
192.168.56.102  item.shopping.com
192.168.56.102  auth.shopping.com
192.168.56.102  cart.shopping.com
192.168.56.102  order.shopping.com
192.168.56.102  member.shopping.com
192.168.56.102  seckill.shopping.com
以上ip换成自己Linux的ip地址
```
...
2、在shopping.conf中添加如下配置
server {
	# 监听以下域名地址的80端口
    listen       80;
    server_name  shopping.com  *.shopping.com hjl.mynatapp.cc;
    ...
    #支付异步回调的一个配置
    location /payed/ {
        proxy_set_header Host order.shopping.com;        #不让请求头丢失
...
- 克隆整个后端项目 `shopping` ，并导入 IDEA 中完成编译
